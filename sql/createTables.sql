-- tables
-- Table: ORDER
CREATE TABLE CMSORDER (
    ORD_ID bigint NOT NULL AUTO_INCREMENT,
    ORD_NUMBER VARCHAR(50) NOT NULL,
    ORD_DESCRIPTION varchar(1000) NOT NULL,
    ORD_DATE datetime NOT NULL,
    ORD_FROM datetime NOT NULL,
    ORD_TO datetime NOT NULL,
    ORD_CLIENT_NAME varchar(100) NOT NULL,
    ORD_CLIENT_NUM int(20) NOT NULL,
    W_ID bigint NULL,
    ORD_CREATED_AT datetime NULL,
    ORD_UPDATED_AT datetime NULL,
    CONSTRAINT ORDER_PK PRIMARY KEY (ORD_ID)
);

-- Table: ROLE
CREATE TABLE ROLE (
    R_ID bigint NOT NULL AUTO_INCREMENT,
    R_ROLE varchar(100) NOT NULL,
    R_DESCRIPTION varchar(1000) NULL,
    R_CREATED_AT datetime NULL,
    R_UPDATED_AT datetime NULL,
    CONSTRAINT ROLE_PK PRIMARY KEY (R_ID)
);

-- Table: SCHEDULE
CREATE TABLE SCHEDULE (
    SCH_ID bigint NOT NULL AUTO_INCREMENT,
    SCH_INTERVAL int NOT NULL,
    SCH_FLAG varchar(50) NOT NULL,
    SCH_DESCRIPTION varchar(1000) NULL,
    WP_ID bigint NULL,
    SCH_CREATED_AT datetime NULL,
    SCH_UPDATED_AT datetime NULL,
    CONSTRAINT SCHEDULE_PK PRIMARY KEY (SCH_ID)
);

-- Table: SKILL
CREATE TABLE SKILL (
    S_ID bigint NOT NULL AUTO_INCREMENT,
    S_NAME varchar(100) NOT NULL,
    S_DESCRIPTION varchar(1000) NOT NULL,
    S_CREATED_AT datetime NULL,
    S_UPDATED_AT datetime NULL,
    CONSTRAINT SKILL_PK PRIMARY KEY (S_ID)
);

-- Table: USER
CREATE TABLE USER (
    U_ID bigint NOT NULL AUTO_INCREMENT,
    U_LOGIN varchar(100) NOT NULL,
    U_PASSWORD varchar(100) NOT NULL,
    U_CREATED_AT datetime NULL,
    U_UPDATED_AT datetime NULL,
    CONSTRAINT USER_PK PRIMARY KEY (U_ID)
);

-- Table: USER_ROLE
CREATE TABLE USER_ROLE (
    UR_ID bigint NOT NULL AUTO_INCREMENT,
    U_ID bigint NOT NULL,
    R_ID bigint NOT NULL,
    CONSTRAINT USER_ROLE_pk PRIMARY KEY (UR_ID)
);

-- Table: WORKER
CREATE TABLE WORKER (
    W_ID bigint NOT NULL AUTO_INCREMENT,
    W_NAME varchar(100) NOT NULL,
    W_TELEPHONE int(20) NOT NULL,
    W_CREATED_AT datetime NULL,
    W_UPDATED_AT datetime NULL,
    CONSTRAINT WORKER_PK PRIMARY KEY (W_ID)
);

-- Table: WORKER_SKILL
CREATE TABLE WORKER_SKILL (
    WS_ID bigint NOT NULL AUTO_INCREMENT,
    S_ID bigint NOT NULL,
    W_ID bigint NOT NULL,
    CONSTRAINT WORKER_SKILL_PK PRIMARY KEY (WS_ID)
);

-- Table: WORKPLAN
CREATE TABLE WORKPLAN (
    WP_ID bigint NOT NULL AUTO_INCREMENT,
    WP_DATE datetime NOT NULL,
    WP_DESCRIPTION varchar(1000) NULL,
    W_ID bigint NULL,
    WP_CREATED_AT datetime NULL,
    WP_UPDATED_AT datetime NULL,
    CONSTRAINT WORKPLAN_PK PRIMARY KEY (WP_ID)
);

-- foreign keys
-- Reference: CMSORDER_WORKER (table: CCMSORDER)
ALTER TABLE CMSORDER ADD CONSTRAINT CMSORDER_WORKER FOREIGN KEY CMSORDER_WORKER (W_ID)
    REFERENCES WORKER (W_ID);

-- Reference: SCHEDULE_WORKPLAN (table: SCHEDULE)
ALTER TABLE SCHEDULE ADD CONSTRAINT SCHEDULE_WORKPLAN FOREIGN KEY SCHEDULE_WORKPLAN (WP_ID)
    REFERENCES WORKPLAN (WP_ID) ON DELETE CASCADE;

-- Reference: USER_ROLE_ROLE (table: USER_ROLE)
ALTER TABLE USER_ROLE ADD CONSTRAINT USER_ROLE_ROLE FOREIGN KEY USER_ROLE_ROLE (R_ID)
    REFERENCES ROLE (R_ID) ON DELETE CASCADE;

-- Reference: USER_ROLE_USER (table: USER_ROLE)
ALTER TABLE USER_ROLE ADD CONSTRAINT USER_ROLE_USER FOREIGN KEY USER_ROLE_USER (U_ID)
    REFERENCES USER (U_ID) ON DELETE CASCADE;

-- Reference: WORKER_SKILL_SKILL (table: WORKER_SKILL)
ALTER TABLE WORKER_SKILL ADD CONSTRAINT WORKER_SKILL_SKILL FOREIGN KEY WORKER_SKILL_SKILL (S_ID)
    REFERENCES SKILL (S_ID) ON DELETE CASCADE;

-- Reference: WORKER_SKILL_WORKER (table: WORKER_SKILL)
ALTER TABLE WORKER_SKILL ADD CONSTRAINT WORKER_SKILL_WORKER FOREIGN KEY WORKER_SKILL_WORKER (W_ID)
    REFERENCES WORKER (W_ID) ON DELETE CASCADE;

-- Reference: WORKPLAN_WORKER (table: WORKPLAN)
ALTER TABLE WORKPLAN ADD CONSTRAINT WORKPLAN_WORKER FOREIGN KEY WORKPLAN_WORKER (W_ID)
    REFERENCES WORKER (W_ID) ON DELETE CASCADE;

-- End of file.
